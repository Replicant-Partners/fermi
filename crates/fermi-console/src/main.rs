...
impl FermiConsole {
    ...
    fn refresh_forecast_shares_cache(&mut self, cx: &mut Context<Self>) {
        ...
        // Collect eligible forecast ids: own forecasts (active + draft +
        // resolved) with share_count > 0 that we haven't already loaded
        // or fired a request for. team_id-owned forecasts also qualify,
        // because they might carry additional team shares beyond the
        // owning one.
    ...
    /// A small coloured dot indicating a forecast's primary team
    /// association. Returns an empty element when there's no team
    ...
    fn render_team_dot(&self, team_id: Option<&str>) -> AnyElement {
        match team_id {
            ...
            Some(tid) => {
                ...
                // Best-effort human-readable label. When teams haven't
                // loaded yet, or the team is one the current user isn't
                // a member of (share-only), fall back to a short id
                // prefix so the operator sees something meaningful.
                ...
                div()
                    ...
                    // Inline label — tiny, dim, only visible when the
                    // row is wide enough. Serves as a quiet legend so
                    // the operator learns the colour → team mapping
    ...
    fn render_activity_item(&self, item: &ActivityItem, cx: &Context<Self>) -> impl IntoElement {
        ...
        // Team association lookup — pulled from own-forecasts, which
        // covers everything the activity feed surfaces (drafts, actives,
        // resolutions). Anything else is None and renders no dot.
