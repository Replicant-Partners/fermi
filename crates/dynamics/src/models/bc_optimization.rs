...
impl BcOptimization {
    ...
    pub fn from_context(
        ...
        Self { k_b, k_ph, k_bc, k_q, ph_floor, bc_max,
               carbon_factor, agit_yield_factor, agit_quality_penalty, do_factor }
    }

    pub fn from_rates(
        k_b: f64, k_ph: f64, k_bc: f64, k_q: f64,
        ph_floor: f64, bc_max: f64,
        carbon_factor: f64, agit_yield_factor: f64,
        agit_quality_penalty: f64, do_factor: f64,
    ) -> Self {
        Self { k_b, k_ph, k_bc, k_q, ph_floor, bc_max,
               carbon_factor, agit_yield_factor, agit_quality_penalty, do_factor }
    }
    ...
    fn manifest(&self) -> ModelManifest {
        ModelManifest {
            ...
            description: "4D model: Brix + pH + BC yield + BC quality. \
                ...
                Anchored on PMC8657668 (Ruka et al.).".into(),
            applies_to_set: vec![
            ...
            params_schema: BTreeMap::from([
                ("bc_max".into(), ParamSchema { label: "BC max".into(), units: "g/L".into(), description: "Carrying capacity.".into(), default: 6.0, typical_range: Some((3.0, 10.0)) }),
            ...
            context_schema: BTreeMap::from([
                ("temperature_c".into(), ContextSchema { label: "Temperature".into(), units: "°C".into(), description: "Arrhenius base.".into(), source: ContextSource::ProcessField { path: "stages[*].temperature_c".into() } }),
                ("agitation_rpm".into(), ContextSchema { label: "Agitation".into(), units: "rpm".into(), description: "0=static, 120-200=agitated.".into(), source: ContextSource::OperatorInput }),
                ("do_saturation_pct".into(), ContextSchema { label: "DO saturation".into(), units: "%".into(), description: "Dissolved oxygen %. Sweet spot ~10%.".into(), source: ContextSource::OperatorInput }),
                ("carbon_source".into(), ContextSchema { label: "Carbon source".into(), units: "name".into(), description: "glucose|sucrose|fructose|glycerol.".into(), source: ContextSource::OperatorInput }),
            ]),
            ...
            citations: vec![
                "Ruka et al. (2012) PMC8657668 — Table 1 + Section 5: BC production optimization".into(),
    ...
    fn generate_notes(&self, trajectory: &[(f64, Vec<f64>)]) -> Vec<Note> {
        ...
        if let Some((t, y)) = trajectory.last() {
            ...
            if yield_val >= self.bc_max * 0.85 {
                notes.push(Note {
                    severity: "info".into(),
                    message: format!("BC yield {:.2} g/L approaching capacity {:.1} g/L.", yield_val, self.bc_max),
                    t_hours: Some(t * 24.0),
                });
            }
        }
...
mod tests {
    ...
    fn agitated_higher_yield_lower_quality() {
        ...
        assert!(dy_a[3].abs() > dy_s[3].abs(), "Agitated should degrade quality faster");
    }
    ...
    fn glucose_better_than_glycerol() {
        ...
        let y = vec![8.0, 4.5, 0.5, 1.0];
        let mut dy_g = vec![0.0; 4]; let mut dy_y = vec![0.0; 4];
        m_gluc.system(0.0, &y, &mut dy_g);
        m_glyc.system(0.0, &y, &mut dy_y);
        assert!(dy_g[2] > dy_y[2], "Glucose should yield more BC than glycerol");
    }
    ...
    fn do_optimum_at_ten_percent() {
        ...
        let y = vec![8.0, 4.5, 0.5, 1.0];
        let mut dy_opt = vec![0.0; 4]; let mut dy_high = vec![0.0; 4];
