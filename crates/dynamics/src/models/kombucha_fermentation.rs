...
impl DynamicsModel for KombuchaFermentation {
    fn manifest(&self) -> ModelManifest {
        ModelManifest {
            ...
            description:
                "Coupled Brix and pH dynamics with Arrhenius temperature dependence and pH floor."
                    .into(),
            ...
            params_schema: BTreeMap::from([
                (
                    "A_b".into(),
                    ParamSchema {
                        label: "Brix pre-exponential".into(),
                        units: "day⁻¹".into(),
                        description: "Arrhenius A for Brix consumption.".into(),
                        default: 2.06e7,
                        typical_range: None,
                    },
                ),
                (
                    "E_b".into(),
                    ParamSchema {
                        label: "Brix activation energy".into(),
                        units: "J/mol".into(),
                        description: "Arrhenius Ea for Brix consumption.".into(),
                        default: 50000.0,
                        typical_range: Some((40000.0, 60000.0)),
                    },
                ),
                (
                    "A_ph".into(),
                    ParamSchema {
                        label: "pH pre-exponential".into(),
                        units: "day⁻¹".into(),
                        description: "Arrhenius A for pH drop.".into(),
                        default: 3.86e6,
                        typical_range: None,
                    },
                ),
                (
                    "E_ph".into(),
                    ParamSchema {
                        label: "pH activation energy".into(),
                        units: "J/mol".into(),
                        description: "Arrhenius Ea for pH drop.".into(),
                        default: 47000.0,
                        typical_range: Some((40000.0, 55000.0)),
                    },
                ),
                (
                    "ph_floor".into(),
                    ParamSchema {
                        label: "pH floor".into(),
                        units: "dimensionless".into(),
                        description: "Minimum pH — acidification stops here.".into(),
                        default: 2.5,
            ...
            context_schema: BTreeMap::from([(
                ...
                ContextSchema {
                    ...
                    units: "°C".into(),
                    description: "Controls Arrhenius rates. Sourced from process stage config."
                        .into(),
    ...
    fn is_converged(&self, history: &[(f64, Vec<f64>)]) -> bool {
        ...
        let ph_range = last.iter().map(|(_, y)| y[1]).fold(f64::INFINITY, f64::min)
            ..=last
                ...
                .map(|(_, y)| y[1])
                .fold(f64::NEG_INFINITY, f64::max);
        (brix_range.end() - brix_range.start()) < 0.01 && (ph_range.end() - ph_range.start()) < 0.01
    }
            ...
            if i > 0 && y[1] <= self.ph_floor + 0.05 {
                ...
                if prev_ph > self.ph_floor + 0.05 {
                    notes.push(Note {
                        severity: "info".into(),
                        message: format!(
                            "pH floor ({:.2}) engaged — acidification stops.",
                            self.ph_floor
                        ),
                        t_hours: Some(t * 24.0),
                    });
        ...
        if let Some((t, y)) = trajectory.last() {
            if y[0] < 0.5 {
                notes.push(Note {
                    ...
                    message: format!(
                        "Brix exhausted ({:.2}%) by day {:.1} — fermentation substrate depleted.",
                        y[0], t
                    ),
                    t_hours: Some(t * 24.0),
                });
...
mod tests {
    ...
    fn arrhenius_higher_temp_faster_rate() {
        ...
        assert!(
            m30.k_b > m20.k_b,
            "Higher temperature should give higher k_b"
        );
        assert!(
            m30.k_ph > m20.k_ph,
            "Higher temperature should give higher k_ph"
        );
    }
