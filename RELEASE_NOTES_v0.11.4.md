...
An intermediate commit (`1e83606d`, "fix(embeddings): realign
embedding identity") picked up a partially-applied v0.11.4 route
declaration on `src/api_server.rs` without the corresponding handler.
`main` at that commit does not compile:

```
error[E0425]: cannot find value `orchestra_manager_effect_handler`
              in module `handlers::orchestras`
```

Shipping v0.11.4 restores the handler and closes the break. The
embedding-identity commit's substantive changes are unaffected.
...
- `crates/fermi-console/src/api/client.rs::CreateForecastRequest`
  ...
  server builds still accept the payload).
...
- Empty state when no forecasts have counterfactuals yet — the
  ...
  the chart is replaced by an explanatory box.
...
`orchestra_xaman_ek_members`. Deferred as a low-risk mig-174. The
current alphabetical order is deterministic and legible.

## Post-deploy verification

```bash
# Item 0: verify main compiles again.
cargo check --bin api-server -p fermi
...
- **Exemplar ordering by `total_executions`** — expose the column
  on `orchestra_xaman_ek_members` (mig-174) so the digest surfaces
...
`src/api_server.rs` already carries the route (picked up early by
the intermediate embedding-identity commit); this release makes it
actually resolve.
