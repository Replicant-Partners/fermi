...
#
# Known gap this surfaces: `apply_settlement` in src/handlers/polymarket.rs
# resolves forecasts with raw SQL and never calls the Spec 31 commit hook,
# and `reconcile_once` in src/handlers/forecast_git.rs INNER JOINs teams,
# so a forecast with no workspace_id can never be picked up by the safety
# net either. Auto-resolved forecasts therefore record nothing for the one
# event that is terminal and unrevertible.
#
...
\echo ''
\echo '=== 6. Resolved post-ship with no commit (should be empty) ==='
SELECT
  left(f.id, 8)                                         AS forecast,
  COALESCE(f.resolution_source, '-')                    AS resolved_via,
