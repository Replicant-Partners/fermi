...
Three views on the same live data. Whether solo flight or rabble, the creature generates telemetry, and these are three lenses:

| View | What it shows | Solo flight | Rabble |
|------|---------------|-------------|--------|
| **Chat + Flock Map** | Narrative + creature positions + dynamics | Creature narrates its own flight (telemetry auto-published to chat) | Multi-creature chat, flock visualization, narrator |
| **Map** | Spatial/geographic — paths, markers, position | Solo path on map with telemetry breadcrumbs | All creatures on map, live positions |
| **AR Viewport** | Immersive spatial — creatures in physical space | Single creature flying in AR | Formation view, portal mode |

The three views are tab-switchable within Zone 3a. The data source is identical (creature_flights.path_samples + rabble_messages) — the views just render it differently.

For **solo flights**, telemetry is periodically published to the creature's personal workspace chat (via the navigator agent). This makes the solo experience feel alive — the creature reports what it observes, habitat descriptions, distance traveled — using the same chat infrastructure as rabbles. A solo flight is a "rabble of one."

| Element | Type | Content |
|---------|------|---------|
| View switcher | Tab bar | Chat / Map / AR |
...
| **Flight dynamics** | **Control surface** | **Available in ALL three views, for any active flight** |
...
### Flight Dynamics — Universal Control Layer

Flight dynamics controls are available on **any active flight**, in **any view** (Chat, Map, AR). They apply to both solo and rabble flights. The difference is who's driving:

| Mode | Who controls | How |
|------|-------------|-----|
| **Manual (solo)** | User directly | Explicit controls: accelerate, decelerate, elevate, descend, bank left/right |
| **Algorithmic (rabble)** | Reynolds model | Separation, alignment, cohesion parameters. Host tweaks knobs. |
| **Hybrid** | User sets constraints, model fills in | User sets max speed, altitude band, heading bias — model handles moment-to-moment |
| **Delegated** | Fully model-driven | User delegates to navigator/reynolds agent entirely |

The control surface is the same set of parameters either way:

| Control | Solo meaning | Rabble meaning | Parameter |
|---------|-------------|----------------|-----------|
| Speed | Direct velocity | Max speed cap for flock | `max_speed` |
| Separation | N/A (solo) | How far apart creatures fly | `separation_radius` |
| Alignment | N/A (solo) | How much creatures match heading | `alignment_weight` |
| Cohesion | N/A (solo) | How tightly creatures cluster | `cohesion_weight` |
| Elevation | Altitude | Altitude band for group | `elevation` (new) |
| Heading | Direct bearing | Heading bias for flock | `heading_bias` (new) |
| Acceleration | Speed change rate | Group acceleration | `acceleration` (new) |

**Key insight:** Reynolds flocking parameters *are* flight dynamics, just abstracted for groups. Making them explicit for solo means exposing the same physics engine without multi-agent context. A solo creature with `max_speed`, `elevation`, and `heading` is a remote-controlled flight. A rabble with `separation`, `alignment`, `cohesion` is an algorithmically-controlled formation. The control surface is unified — the context determines which knobs are relevant.

**Delegation model:** The user can choose how much control to retain:
- Full manual: every adjustment is explicit (like a remote control)
- Constrained delegation: set bounds, let the model fly within them
- Full delegation: hand off to the navigator agent, sit back and watch the narration

This maps cleanly to the existing infrastructure:
- `POST /api/rabble/:id/flock` already accepts Reynolds parameters — extend to accept speed/elevation/heading for solo
- Navigator agent already narrates flights — extend to respond to dynamic changes ("Creature accelerating over the channel...")
- `creature_flights.path_samples` already records position — extend to record altitude, speed, heading per sample
...
│ ● In flight at London       │
│   Duration: 12m | [End]     │
│   Navigator: "Passing over  │
│   Thames estuary, spotted   │
│   3 species of wader..."    │
