...
| Fund creature | Creature wallet | Credits for creature actions |
...
The creature is the **agent** that performs all runtime actions. It invites, plans, teleports, hosts, joins, leaves, coordinates, and chats. The creature is the proxy for all activity.

| Action | Cost | Prerequisite | Notes |
|--------|------|--------------|-------|
| Invite to rabble | free | Active creature | Creature extends invitation |
...
graph TB
    ...
    subgraph "Layer 2: CREATURE — Actions"
        C1 -->|invites| INV[Invite to Rabble]
...
3. **Credits are User-level.** The user's wallet pays for all creature actions.
4. **Creature is the social identity in rabbles.** Chat messages, flock positions, attraction scores — all tied to creature, not user.
5. **Everything below actions is reporting.** Flight Log, Flock History, Leaderboard, AR Portal — these are views on data that creatures generated through actions.
6. **User Profile = roll-up.** Aggregate view across all owned creatures: total flights, rabbles hosted/joined, social interactions, activity timeline.
7. **Explore = global view.** All visible flights + active rabbles, filterable by proximity, species, status.
