...
pub enum DreamMaturity {
    ...
    Dormant,
    /// The agent has no episodes to consolidate — it was never really executed.
    /// Distinct from [`DreamMaturity::Unproductive`]: there is nothing to learn
    /// from, so learning nothing is the correct outcome, not a fault.
    Unused,
    /// Episodes existed and were consolidated, but produced no durable
    /// knowledge. This IS a fault.
    Unproductive,
...
pub struct MaturityInputs {
    ...
    /// Total episodes the agent has ever produced, consolidated or not.
    ///
    /// Without this the classifier cannot tell "consolidated a real workload
    /// and extracted nothing" (a fault) from "had nothing to consolidate"
    /// (an unused agent behaving correctly). Measured against the real fleet,
...
pub fn classify_maturity(i: MaturityInputs) -> (DreamMaturity, String) {
    ...
    if i.ontology_size() == 0 && i.total_episodes == 0 {
        // Nothing to learn from. Consolidation ran (or was seeded) but there
        // were no episodes, so an empty ontology is the correct result. Calling
        // this a fault buries the genuine faults in noise.
        return (
            DreamMaturity::Unused,
            "Has never produced an episode, so there is nothing to consolidate. \
             Not a dreaming fault — the agent needs to be executed before Loop 1 \
             has any material to work with."
                .to_string(),
        );
    }

    if i.ontology_size() == 0 {
        // Episodes existed and were still not turned into knowledge. This is
        // the silent failure the module exists to surface.
        return (
...
pub async fn fleet_dreaming_maturity_handler(
    ...
    let (mut dormant, mut unused, mut unproductive, mut developing, mut mature) = (0, 0, 0, 0, 0);
    ...
    Ok(Json(json!({
        ...
        "counts": {
            "dormant": dormant,
            // Idle, not broken: no episodes to consolidate. The common case.
            "unused": unused,
        ...
        "agents": agents,
        "note": "`unproductive` counts agents that consolidated REAL episodes and still produced no entities, facts or rules — a loop that runs and learns nothing. That is a fault. `unused` agents simply have no episodes yet: an empty ontology is the correct result and not a fault.",
    })))
}
...
mod tests {
    ...
    fn only_failures_is_dormant_but_says_so() {
        ...
        assert!(
            why.contains("credential") || why.contains("budget"),
            "{why}"
        );
    }
    ...
    /// The failure this module exists for: the loop runs on REAL episodes,
    /// charges credits, advances its timestamp, and learns nothing.
    #[test]
    fn cycles_over_real_episodes_without_yield_are_unproductive() {
        let (band, why) = classify_maturity(MaturityInputs {
            completed_cycles: 50,
            total_episodes: 400,
            ..Default::default()
    ...
    /// ontology is correct rather than broken. Measured against the real fleet
    /// this is the overwhelmingly common case (537 of 731 agents have zero
    /// episodes); reporting it as a fault buried the genuine faults in noise.
    #[test]
    fn cycles_without_any_episodes_are_unused_not_unproductive() {
        let (band, why) = classify_maturity(MaturityInputs {
            completed_cycles: 3,
            total_episodes: 0,
            ..Default::default()
        });
        assert_eq!(band, DreamMaturity::Unused);
        assert!(why.contains("never produced an episode"), "{why}");
        assert!(
            !why.contains("fault") || why.contains("Not a dreaming fault"),
            "must not read as a fault: {why}"
        );
    }
