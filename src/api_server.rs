...
async fn get_episode_detail_handler(
...
async fn platform_metrics_handler(
    State(state): State<AppState>,
) -> Result<Json<Value>, (StatusCode, String)> {
    let pool = &state.db;
...
async fn list_eval_test_cases_handler(
    State(state): State<AppState>,
    Path(agent_id): Path<String>,
) -> Result<Json<Value>, (StatusCode, String)> {
    let db_agent = resolve_agent(&state, &agent_id).await?;
...
async fn create_eval_test_case_handler(
    State(state): State<AppState>,
...
async fn update_eval_test_case_handler(
    State(state): State<AppState>,
    principal: AuthPrincipal,
...
async fn delete_eval_test_case_handler(
    State(state): State<AppState>,
    principal: AuthPrincipal,
    Path((agent_id, test_case_id)): Path<(String, String)>,
) -> Result<Json<Value>, (StatusCode, String)> {
...
async fn trigger_eval_run_handler(
    State(state): State<AppState>,
    principal: AuthPrincipal,
    Path(agent_id): Path<String>,
...
async fn main() {
    ...
    let public_routes = Router::new()
        ...
        .route("/api/episodes/:episode_id", get(get_episode_detail_handler))
        .route("/api/metrics/platform", get(platform_metrics_handler))
        .route("/api/agents/:agent_id/metrics", get(agent_metrics_handler))
        // Eval framework
        .route(
            "/api/agents/:agent_id/eval/test-cases",
            get(list_eval_test_cases_handler),
        )
        .route(
            "/api/agents/:agent_id/eval/test-cases",
            post(create_eval_test_case_handler),
        )
        .route(
            "/api/agents/:agent_id/eval/test-cases/:test_case_id",
            put(update_eval_test_case_handler),
        )
        .route(
            "/api/agents/:agent_id/eval/test-cases/:test_case_id",
            delete(delete_eval_test_case_handler),
        )
        .route(
            "/api/agents/:agent_id/eval/run",
            post(trigger_eval_run_handler),
        )
        .route(
            "/api/agents/:agent_id/eval/runs",
            get(list_eval_runs_handler),
        )
        .route(
